...
config_options()
{
    ...
    desc.add_options()("port,p", po::value<std::uint16_t>()->required(),
                       "override configured port number on which "
                       "the server listens for incoming connections");
    desc.add_options()("ipaddress,i", po::value<std::string>()->required(),
                       "override configured ip number on which "
                       "the server listens for incoming connections");
    desc.add_options()("mountpoint,m", po::value<std::string>()->required(),
                       "override path to folder where the file system "
                       "is to be mounted");

    // adds options that can be passed only via the CLI
    desc.add_options()("help,h", "display this message and exit")(
        "config,c",
...
{
    std::cout << "Usage: sadfsd-bootstrap OPTIONS\n\n"
              << "Options:\n"
              << options << '\n';
}
...
main(int argc, char const** argv)
{
    ...
    auto sadfilesys =
        sadfs::sadfilesys{variables["ipaddress"].as<std::string>().c_str(),
    ...
    std::transform(filesys_args.begin(), filesys_args.end(),
                   filesys_argv.begin(), get_ptr);

    auto status =
        sadfilesys.bootstrap(filesys_args.size(), filesys_argv.data());
