...
auto fmt_err = [](char const *msg, ip_addr const &ip, port_no const &port) {
    return ""s // explicitly tell the compiler that we want a string
           + msg + to_string(ip) + ":" + std::to_string(to_int(port));
};
 ...
 * -------------------------------------------------------------
 */
ip_addr::ip_addr(char const *ip) : value_(parse_ip_addr(ip))
{
    // do nothing
}

std::uint32_t
ip_addr::value() const noexcept
{
    return value_;
}
 ...
 * -------------------------------------------------------------
 */
port_no::port_no(int port) : value_(parse_port_no(port))
{
    // do nothing
}

std::uint16_t
port_no::value() const noexcept
{
    return value_;
}
 ...
 * -------------------------------------------------------------
 */
service::service(char const *ip, int port) : ip_(ip), port_(port)
{
    // do nothing
}

ip_addr
service::ip() const noexcept
{
    return ip_;
}

port_no
service::port() const noexcept
{
    return port_;
}

socket
service::connect() const noexcept
{
    // create a socket
    ...
    if (::connect(sock.descriptor(), reinterpret_cast<sockaddr const *>(&addr),
                  sizeof(addr)) == -1)
    {
 ...
 * -------------------------------------------------------------
 */
listener::listener(service const &s)
    : service_{s}, socket_{socket::domain::inet, socket::type::stream}
{
    if (!socket_.valid())
    {
    ...
    if (::bind(socket_.descriptor(), reinterpret_cast<sockaddr const *>(&addr),
               sizeof(addr)) == -1)
    {
